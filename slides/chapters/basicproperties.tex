...
\begin{frame}{Introduction}
    The proofs will use the Hahn-Banach corollaries a couple of times.
    So first, we need to recall Hahn-Banach related theorems.
\end{frame}

\begin{frame}{Revision: Hahn-Banach}
    \begin{block}{Revision (Hahn-Banach)}
...
\begin{frame}{Revision: Dual Unit Ball}
    \begin{block}{Revision}
        The dual unit ball retains information about the norm of primal vectors: \\
        Let $(X, \norm{}{\cdot})$ be a normed \thefield\ vector space and $x \in X$. \\
        Then we have \[\norm{X}{x} = \sup_{f \in X', \norm{}{f} \le 1}\abs{f(x)}\]
    \end{block}
\end{frame}

\begin{frame}{Revision: Dual Unit Ball -- Proof (Case 1)}
    \begin{proof}
...
\begin{frame}{Revision: Dual Unit Ball -- Proof (Case 2)}
    \begin{proof}
        \begin{itemize}
            \item Case 2 $x \neq 0$:
                We have
...
\begin{frame}{Theorem: Properties of Adjoint Operator}
    \begin{theorem}
        ...
        \begin{enumeratetheorem}
            \item $T' \in \contoperators{Y'}{X'}$, so $T'$ is linear and bounded. \\
                \sidenote{This implies $\forall y' Y': T'y' \in X'$.}
            \item $T \mapsto T'$ is linear and isometric.
        \end{enumeratetheorem}
    \end{theorem}
\end{frame}

\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (i) Image}
    \begin{proof}
...
\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (i) Linearity}
    \begin{proof}
        Let $y'_1, y'_2 \in Y', \alpha \in \thefield$. We then prove linearity:
        \begin{align*}
            T'(\alpha y'_1 + y'_2)
...
\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (i) Boundedness}
    \begin{proof}
        Let $y' \in Y'$. We then prove boundedness of the operator norm:
        \begin{align*}
            \norm{X'}{T' y'}
...
\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (ii) Linearity}
    \begin{proof}
...
\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (ii) Isometry}
    \begin{proof}
        We then prove isometry:
        \begin{align*}
            \norm{}{T}
            &= \sup_{\norm{X}{x} \le 1}\norm{Y}{Tx}                                 \explainstep{use supremum char. of op. norm} \\
            &= \sup_{\norm{X}{x} \le 1}\sup_{\norm{}{y'} \le 1}{\abs{y'(Tx)}}       \explainstep{apply theorem of Hahn-Banach} \\
...
\begin{frame}{Example: Shift Operator -- Setup}
    \begin{example}
...
\begin{frame}{Example: Shift Operator -- Proof Idea for $\lp' \isomorphto \lpconj$}
    \begin{example}
        ...
        \begin{itemize}
            ...
            \item Verify $T$ is surjective and isometric (the long part). See\ \cite{werner_funktionalanalysis_2018} for a full proof.
        \end{itemize}
    \end{example}
\end{frame}

\begin{frame}{Example: Shift Operator -- Left Shift}
    \begin{example}
        To illustrate the adjoint operator, we now work through an example.
...
\begin{frame}{Example: Shift Operator -- Adjoint Signature}
    \begin{example}
        We can now compute the adjoint operator $T'$:
        \begin{itemize}
            \item
...
\begin{frame}{Example: Shift Operator -- Adjoint Calculation}
    \begin{example}
...
\begin{frame}{Example: Shift Operator -- Conclusion}
    \begin{example}
        This tells us that the adjoint operator $T'$ acts as a \highlight{right shift} (up to isomorphism):
        \[T': \lpconj \rightarrow \lpconj, \sequence{k}{s_k} \mapsto (0, s_1, s_2, \dots)\]
    \end{example}
\end{frame}

\begin{frame}{Theorem: Adjoint Reverses Composition}
    \begin{theorem}
...
\begin{frame}{Theorem: Adjoint Reverses Composition -- Proof}
    \begin{proof}
...
\begin{frame}{Example: Composition Rule}
    \begin{example}
        Referring back to the example with $A \in \reals^{m \times n}$,
        the adjoint operator reverses composition rule is compatible
        with the way matrix transposition on \reals
        or the matrix adjoint on \complexes behave.
    \end{example}
\end{frame}
