...
\begin{frame}{Revision: Hahn-Banach and Dual Unit Ball}
    \begin{block}{Revision (Hahn-Banach)}
        Let $(X, \norm{X}{\cdot})$ be a normed space and $0 \neq x \in X$. \\
        Then we have $\exists f \in X': \norm{X'}{f} = 1 \logicaland f(x) = \norm{X}{x}$
    \end{block}
...
\begin{frame}{Theorem: Properties of Adjoint Operator -- Proof (ii) Isometry}
    \begin{proof}
        ...
        \begin{align*}
            ...
            &= \sup_{\norm{X}{x} \le 1}\sup_{\norm{}{y'} \le 1}{\abs{y'(Tx)}}       \explainstep{apply Hahn-Banach corollary} \\
...
\begin{frame}{Example: Shift Operator (1/3)}
    \begin{example}
...
\begin{frame}{Example: Shift Operator (2/3)}
    \begin{example}
...
\begin{frame}{Example: Shift Operator (3/3)}
    \begin{example}
