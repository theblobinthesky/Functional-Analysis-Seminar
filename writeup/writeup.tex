...
\begin{abstract}
    This proseminar paper provides a foundational overview of dual-/adjoint operators in functional analysis.
\end{abstract}
...
\begin{remark}
    In comparison to the operators we have previously worked with, the adjoint operator takes and outputs linear, bounded operators.
	 ...
	 For $y' \in Y' = \contoperators{Y}{\thefield}$, the adjoint operator evaluates to $T' y' \in \contoperators{X}{\thefield}$.
\end{remark}

\noindent First, we need to recall a corollary of the Hahn-Banach theorems:
\begin{revision}
...
\begin{proof}
    ...
    \begin{align*} 
        ...
        \le 1 \norm{X}{x} = \norm{X}{x}
    \end{align*}
    By Hahn-Banach, we have $\exists f \in X': \norm{}{f} = 1 \logicaland \abs{f(x)} = \norm{X}{x}$. \\
...
\begin{theorem}
    ...
    Let $T \in \contoperators{X}{Y}$ be a bounded linear operator and $T' \in \contoperators{Y'}{X'}$ the adjoint operator. \\
...
% \newpage
% \subsection{Examples for Adjoint Operators on Hilbert Spaces}

% \newpage
% \subsection{Examples for Adjoint Operators in Finite Dimensional Hilbert Spaces}
...
\begin{corollary}
    Let $T \in \contoperators{X}{Y}$ be a linear, continous operator with $\im T$ closed.
...
% TODO: Add script from ss25 in references list.
