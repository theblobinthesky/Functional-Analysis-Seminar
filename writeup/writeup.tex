...
% ===== MISC =====
\newcommand{\refertolecture}{\text{Please refer to the functional analysis lecture notes from the summer of 2025.}}
...
\begin{theorem}\label{theorem:bidual-adjunct-embedding}
    ...
    \comment{Or equivalently: $T|_{J_X(X)} = J_Y \chain T'' \chain \inv{J_X|_{J_X(X)}}$.} \\
    \comment{Or equivalently: $T'' = \inv{J_Y} \chain T \chain J_X$.}
\end{theorem}
\begin{proof}
...
\begin{revision}\label{theorem:embedding-isomorphism}
    ...
    \begin{romanenum}
        ...
        \item TODO JXinv is basically a bounded, linear operator. I just hope that's true m'fucka!
    \end{romanenum}
\end{revision}
\begin{proofidea}
...
% TODO: Explain the invertible embedding functions J_X and J_Y (e.g. J_X^{-1} and J_Y^{-1}).
