...
to general operators on normed vector spaces:
...
\begin{revision} \label{revision:general-revision}
    ...
    \begin{romanenum}
        \item The set of continous, linear operators \contoperators{X}{Y} is a Banach space iff.\ $Y$ is a Banach space. \\
...
\begin{definition}
    Let $X, Y$ be normed vector spaces and $T \in \contoperators{X}{Y}$. \\
	Then $T': Y' \rightarrow X', y' \mapsto y' \chain T$ is called the adjoint operator.
    From now on, we will implicitly refer to the normed vector spaces $X, Y$ and the topological dual spaces $X', Y'$ when talking about the dual operator $T'$.
\end{definition}
...
\begin{example}
    ...
    \begin{align*}
        T': \lpconj \rightarrow \lpconj, \sequence{k}{s_k} \mapsto \sequence{}{0, s_1, s_2, \cdots}
    \end{align*}
\end{example}
...
\begin{definition} % NOTE: Revision.
    ...
    \begin{romanenum}
        ...
        \item $T \in \contoperators{X}{Y}$ is compact, if $T B(0, \le 1, X) \text{ is relatively compact}$.
