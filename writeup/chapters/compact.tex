...
\begin{proof}
    ...
    \noindent Using the definitions of $f_n$ and $f_m$ we get
    \[ \norm{\infty}{f_n - f_m} = \sup_{\norm{X}{x} = 1}{((y'_n \chain T) - (y'_m \chain T))x} \]
    The convergence of $f$ happens if and only if is a Cauchy sequence in the Banach space $(\dualspace{Y}, \norm{}{\cdot})$.
    Therefore the convergence of $f$ in the operator norm is only dependent on the behaviour on the closed unit ball.
