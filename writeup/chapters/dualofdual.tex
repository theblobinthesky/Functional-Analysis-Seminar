...
\begin{theorem}\label{theorem:bidual-adjunct-embedding}
    ...
    This concept is illustrated in figure\ \ref{figure:dual-of-dual}.
\end{theorem}
\begin{proof}
...
\noindent We will now characterize when a continuous operator between $Y'$ and $X'$ is an adjoint operator.
We need to revise an important corollary from the lecture first.

\begin{revision}\label{revision:embedding-isomorphism}
...
\begin{proofidea}
    \begin{enumerateproof}
        ...
        \item
            ...
            \begin{itemize}
                \item $J_X$ is linear since the $x' \in X'$ are linear.
                \item $\norm{}{J_X} = 1$.  This means $J_X$ is bounded.
            \end{itemize}
...
\begin{corollary} \label{corollary:bidual-adjunct-embedding-ext}
    We can strengthen\ theorem \ref{theorem:bidual-adjunct-embedding} using the isomorphism from revision\ \ref{revision:embedding-isomorphism}: \\
    Let $(X, \norm{X}{\cdot})$ and $(Y, \norm{Y}{\cdot})$ be normed \thefield\ vector spaces.
    Let $T \in \contoperators{X}{Y}$ be a bounded linear operator. 
    Then we have
    \begin{enumeratetheorem}
        \item $J_Y \chain T = T'' \chain J_X$.
        \item $T''|_{J_X(X)} = J_Y \chain T \chain \canonicalembinv{X}$
        \item $T = \canonicalembinv{Y} \chain T'' \chain J_X$.
    \end{enumeratetheorem}
\end{corollary}
\begin{proof}
    \begin{enumerateproof}
        \item This is the statement of theorem\ \ref{theorem:bidual-adjunct-embedding}.
        \item The isomorphism from revision\ \ref{revision:embedding-isomorphism} gives us the result.
        \item The isomorphism from revision\ \ref{revision:embedding-isomorphism} gives us the result.
    \end{enumerateproof}
\end{proof}
...
\begin{proof}
    ...
    We have $S'(J_X(X)) \subset J_Y(Y)$ and revision\ \ref{revision:embedding-isomorphism} gives us $J_Y(Y) \isomorphto Y$. \\
    ...
    \noindent We know $T$ is linear and continuous, as 
...
% TODO: Maybe i'll add this tomorrow?
% \begin{theorem}
%     Lastly, we get one more property of the adjoint:
%     $T \mapsto T'$ is not always surjective. \\
%     \note{This is not the case with the transpose between $\reals^n \rightarrow \reals^n$.}
% \end{theorem}
% \begin{proof}
